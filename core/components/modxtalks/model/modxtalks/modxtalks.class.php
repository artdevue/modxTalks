...
class modxTalks {
    ...
    public function init($limit = '', $id = '') {
        ...
        $this->config['slug'] = substr($this->modx->getOption('base_url'),1).$this->aliasPath($this->config['slugReplace']);
    ...
    public function getComments($conversation = '', $limit = 20, $id = '') {
        ...
        if ($this->config['debug'] && $this->modx->getOption('log_target') === 'HTML') {
            $this->pr('Всего страниц: '.$totalPages);
        }
    ...
    public function getScrubber($conversationId = 0) {
        ...
        $scrubber = array(
            ...
            'modxtalks_total' => $this->modx->lexicon('modxtalks_total'),
        );
        /**
    ...
    private function _getForm() {
        if ($this->config['onlyAuthUsers'] && !$this->modx->user->isAuthenticated($this->modx->context->key) && !$this->isModerator()) {
        ...
        if ($this->modx->user->isAuthenticated($this->modx->context->key) || $this->isModerator()) {
    ...
    public function userButtons($userId = 0, $time = 0) {
        ...
        if ($this->config['onlyAuthUsers'] && !$this->modx->user->isAuthenticated($this->modx->context->key)) {
            return '';
        }
        return '';
    }
    ...
    public function get_client_ip($type = 'ip') {
        $ip = $this->modx->request->getClientIP();
        return array_key_exists($type, $ip) ? $ip[$type] : '0.0.0.0';
    }
    ...
    public function getCommentsArray($ids,$conversationId) {
        ...
        if ($cCache === false) {
            ...
            $c->select(array('id','idx','content','userId','time','deleteTime','deleteUserId','editTime','editUserId','username','useremail'));
     ...
     */
    public function generateLink($cid,$idx) {
        $conversation = $this->modx->getObject('modxTalksConversation',$cid);
        $rid = $conversation->rid;
        $url = $this->modx->makeUrl($rid,$this->context,'','full');
...
class modxTalks {
     ...
     */
    public function init($limit = '', $id = '') {
        $this->pr($this->config['commentsPerPage']);
        // if ($this->config['debug'] && $this->modx->getOption('log_target') === 'HTML') {
            $time = microtime(true);
            $startMemory = memory_get_usage()/pow(1024,2);
            $this->pr(sprintf('Memory Before: %2.2f Mbytes',$startMemory));
