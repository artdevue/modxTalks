...
jQuery.MTAjax = function (options) {
	...
	var handlerSuccess = function (result, textStatus, XMLHttpRequestObject) {
		...
		if (typeof result.messages == "undefined" || result.messages.length < 1) {
...
var MTConversation = {
	...
	initReply: function () {
		...
		textarea.TextAreaExpander(100, 700);
	...
	editPost: function (postId) {
		...
		jQuery.MTAjax({
			...
			success: function (data) {
				...
				textarea.TextAreaExpander(100, 700).focus().selectRange(len, len);
		...
		if (preview) {
			...
			jQuery.MTAjax({
				...
				success: function (data) {
					...
					jQuery("#" + id + "-bg").hide();
					jQuery("#" + id + "-preview").html(data.message.content).show();
					jQuery('pre').each(function (i, e) {
						hljs.highlightBlock(e)
					});
				}
