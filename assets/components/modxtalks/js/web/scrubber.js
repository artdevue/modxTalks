...
init: function() {
	...
	$(window).scroll(function() {
		...
		if (y+MT.scrubberOffsetTop >= MTScrubber.body.offset().top && !MT.disableFixedPositions) {
			MTScrubber.body.css({paddingTop: MTScrubber.header.outerHeight()});
			MTScrubber.header.addClass("floating").css({position: "fixed", top: MT.scrubberOffsetTop, width: headerWidth, zIndex: 110});
			MTScrubber.items.addClass("scrubtop");
		}
		// Otherwise, put it back to normal.
		else {
			MTScrubber.body.css({paddingTop: 0});
			MTScrubber.header.removeClass("floating").css({position: "", top: "", width: ""});
		...
		if (y+MT.scrubberOffsetTop >= MTScrubber.body.offset().top && !MT.disableFixedPositions) {
			if($('#conversationBody').hasClass('scrubber-top')){
				topofs = MT.scrubberOffsetTop;
			}else{
				topofs = MT.scrubberOffsetTop + 10; //MTScrubber.header.outerHeight() + 10;
			}		 
			MTScrubber.scrubber.addClass("floating").css({position: "fixed", top: topofs, zIndex: 100});
			MTScrubber.items.addClass("scrubtop");
		}
		// Otherwise, put it back to normal.
		else {
			MTScrubber.scrubber.removeClass("floating").css({position: "", top: ""});
			MTScrubber.items.removeClass("scrubtop");
		}
		...
		$(".mtComment", MTScrubber.items).each(function() {
			var item = $(this);
			...
			if (y > item.offset().top + item.outerHeight() - MTScrubber.header.outerHeight()) return true;
				...
				$(".scrubber-"+index, MTScrubber.scrubber).addClass("selected").parents("li").addClass("selected");
				return false;
		...
		var newer = $(".scrubberNext", MTScrubber.body);
		if (newer.length && y + $(window).height() > newer.offset().top && !newer.hasClass("loading") && !MT.disableFixedPositions) {
			newer.find("a").click();
		}
	});
	...
	$(".scrubberMore a", MTScrubber.body).live("click", function(e) {
		e.preventDefault();
		...
		// If this is a "load more" block...
			...
			position = backwards ? $(this).parent().data("positionEnd") - MTScrubber.perPage + 1 : $(this).parent().data("positionStart");
		}
	...
	$(".scrubber a", MTScrubber.body).click(function(e) {
		e.preventDefault();
		...
		if (!found) {
			...
			var moreItem = null,
				prevPost = null;
			$("div", MTScrubber.items).each(function() {
			...
			// 3. If a "more" block wasn't found, and a previous item was found, scroll to the previous item.
				...
				MTScrubber.scrollTo(prevPost.offset().top);
			...
			// 4. If a "more" block WAS found, scroll to it, and load the items.
				...
				MTScrubber.loadItemsCallback(index, function(addIt) {
					...
					if (index == Infinity) var scrollOffset = MTScrubber.items.offset().top + MTScrubber.items.outerHeight() - $(document).scrollTop();

					MTScrubber.addItems(addIt.startFrom, addIt.view, moreItem);

					// Restore the scroll position, or scroll to the index which we should now have items for.
					if (index == Infinity) $.scrollTo(MTScrubber.items.offset().top + MTScrubber.items.outerHeight() - scrollOffset);
					else MTScrubber.scrollToIndex(index);
...
scrollTo: function(position) {
	$.scrollTo(position - MT.scrubberOffsetTop - MTScrubber.header.outerHeight() + 1, "slow");
},
...
scrollToIndex: function(index) {
	...
	// Scroll to it.
	if (item) MTScrubber.scrollTo($(item).offset().top);

	if (typeof MTScrubber.scrollToIndexCallback == "function") MTScrubber.scrollToIndexCallback(index);
...
addItems: function(startFrom, items, moreItem, animate) {
	...
	if ($("div.timeMarker[data-now]", MTScrubber.body).length) {
	...
	var scrubberMore = $("<div class='scrubberMore'><a href='#'>"+MTScrubber.moreText+"</a></div>");
	...
	if (MTScrubber.loadedItems.indexOf(startFrom - 1) == -1 && items.first().prev().is("div:not(.scrubberMore)")) {
	...
	if (MTScrubber.loadedItems.indexOf(startFrom + items.length) == -1 && items.last().next().is("div:not(.scrubberMore)")) {
		scrubberMore = scrubberMore.clone();
	...
	for (var i = startFrom ; i < startFrom  + items.length; i++) {
		if (MTScrubber.loadedItems.indexOf(i) == -1)
			MTScrubber.loadedItems.push(i);
	}

	// If we have the very first item in the collection, remove the "older" block.
	if (Math.min.apply(Math, MTScrubber.loadedItems) <= 1)
