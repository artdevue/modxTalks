...
Ext.extend(MODx.ModxTalksUnconfirmedViewPanel,MODx.DataView,{
    ...
    ,showDetails : function(){
        ...
        if(selNode && selNode.length > 0){
            ...
            if (data) { Ext.getCmp('modxtalks-comment-item-details').updateDetail(data); }
        } else {
...
Ext.extend(modxTalks.panel.unConfirmed,MODx.Panel,{
            ...
            +'<tpl for=".">'
                ...
                +'<div class="spbutmt">'
                    +'<ul class="splitbuttons">'
                        ...
                        +'<li class="inline-button delete"><button ext:qtip="'+_('modxtalks.delet_comment')+'" ext:trackMouse=true ext:anchorToTarget=false" onclick="Ext.getCmp(\'modxtalks-talks-comment\').deleteComment(\'{id}\'); return false;">'+_('modxtalks.delet')+'</button></li>'
                    +'</ul>'
                +'</div>'
    ...
    ,deleteComment: function(id){
        MODx.msg.confirm({
            title: _('modxtalks.delet_comment')
            ,text: _('modxtalks.delet_comment_desc')
            ,url: modxTalks.config.connectorUrl
            ,params: {
