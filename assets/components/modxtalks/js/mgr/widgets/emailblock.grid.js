modxTalks.grid.emailBlock = function (config) {
	...
	Ext.applyIf(config, {
		...
		columns: [this.sm, {
			...
			header: '<img src="' + modxTalks.config.cssUrl + '../img/mgr/16-Tools.png" alt="' + _('modxtalks.delet_conversation') + '" class="modxtalks-email-col-header" />',
			width: 10,
			align: 'center',
			renderer: {
				fn: this._renderEmailDelete,
				scope: this
			}
		}],
		tbar: [{
...
Ext.extend(modxTalks.grid.emailBlock, MODx.grid.Grid, {
	...
	_makeTemplatesEmail: function () {
		...
		+ '<tpl for="actions">'
		+ '<li><a href="#homeTab:email-blocking"><img class="controlBtn deleteemail" src="' + modxTalks.config.cssUrl + '../img/mgr/24-Trashcan_b.png" ext:qtemail="{text}" /></a></li>'
		+ '</tpl>'
		+ '</ul>'
		+ '</tpl>', {
			compiled: true
