modxTalks.grid.ipBlock = function (config) {
	...
	Ext.applyIf(config, {
		...
		columns: [this.sm, {
			...
			header: '<div class="ip-h">' + _('modxtalks.ip_adress') + '</div>',
			dataIndex: 'ip',
			id: 'ip',
			width: 30,
			sortable: true,
...
Ext.extend(modxTalks.grid.ipBlock, MODx.grid.Grid, {
	...
	_makeTemplates: function () {
		...
		+ '<tpl for="actions">'
		+ '<li><a href="#homeTab:ip-blocking"><img class="controlBtn deleteip" src="' + modxTalks.config.cssUrl + '../img/mgr/24-Trashcan_b.png" ext:qtip="{text}" /></a></li>'
		+ '</tpl>'
		+ '</ul>'
		+ '</tpl>', {
			compiled: true
